...
pub mod api {
    ...
    pub trait Api {
        ...
        #[doc(alias = "sys::ffi::playdate_graphics::makeFontFromData")]
        #[inline(always)]
        fn make_font_from_data(&self)
                               -> unsafe extern "C" fn(data: *mut LCDFontData, wide: c_int) -> *mut LCDFont
        {
            // SDK 3.1.0 added a `datalength` parameter to `makeFontFromData`
            // and kept the old 2-arg signature available as `_deprecated`,
            // which is what this crate's API still expects.
            *sys::api!(graphics.makeFontFromData_deprecated)
        }
