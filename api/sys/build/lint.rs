...
pub fn panic_recover_hints_no_builtin(sdk: &str) -> ! {
    panic!("Builtin bindings for SDK {sdk} that covers requested feature-set not found. {}\t{}\t{}\t{}",
           format_args!("Possible options to solve it:"),
           format_args!("- update '{PKG_NAME}' crate with `cargo update`"),
           format_args!("- get '{BIN_NAME}' with `cargo install playdate-bindgen`"),
