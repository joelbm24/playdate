...
impl<Api: api::Api> System<Api> {
    ...
    pub fn set_serial_message_callback<F>(&self, callback: Option<F>)
        where F: 'static + FnMut(String) + Sized + Send {
        use core::ffi::c_char;
        use core::ffi::CStr;
        use alloc::boxed::Box;
        use alloc::string::String;
        ...
        static STORE: Mutex<Option<Box<dyn FnMut(String) + Send>>> = Mutex::new(None);

        pub unsafe extern "C" fn proxy_serial_message_callback<F: FnMut(String)>(data: *const c_char) {
            let data = unsafe { CStr::from_ptr(data as _).to_string_lossy().into_owned() };
            if let Some(ref mut f) = *STORE.lock() {
                f(data)
            } else {
...
pub mod api {
    ...
    impl Api for Cache {
        ...
        fn get_language(&self) -> unsafe extern "C" fn() -> PDLanguage {
            self.0.getLanguage.expect("getLanguage")
        }
    ...
    pub trait Api {
        ...
        #[inline(always)]
        fn get_battery_voltage(&self) -> unsafe extern "C" fn() -> c_float {
            *sys::api!(system.getBatteryVoltage)
        }
