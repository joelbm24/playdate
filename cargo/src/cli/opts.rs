...
fn shorthands_for(cmd: &Cmd) -> Vec<Arg> {
    match cmd {
        Cmd::Build => {
            vec![flag_playdate_device(),
                 flag_playdate_simulator(),
                 flag_no_unwinding()]
        },
        Cmd::Run => {
            vec![flag_playdate_simulator().conflicts_with_all(["device", "mounting"]),
                 flag_no_unwinding()]
        },
...
fn mount() -> Vec<Arg> {
    let mount: Command = Mount::augment_args(Command::new("mount")).mut_arg("device", |arg| {
...
fn flag_create_deps_sys_only() -> Arg {
    let name = "sys-only";
    let help =
        r#"Add only "playdate-sys" dependency, use low-level template, don't add high-level dependencies."#;
    Arg::new(name).long(name).help(help).action(ArgAction::SetTrue)
}
