...
fn main() -> CargoResult<()> {
    ...
    execute(&config).map_or_else(|err| {
        ...
        cli::cmd::Cmd::Run => {
                                   ...
                                   let dropped =
                                       targets.extract_if(.., |target| {
                                                  ...
                                                  !if ck.is_playdate() {
                                                      ...
                                                      cts.contains(&CrateType::Dylib) ||
                                                      cts.contains(&CrateType::Cdylib)
                                                  } else {
                                              ...
                                              .inspect(|target| {
                                                  let cks = format!("as incompatible with {}", match ck {
                                                      CompileKind::Target(ct) => ct.short_name().to_owned(),
                                                      CompileKind::Host =>
                                                          config.host_target.short_name().to_owned(),
                                                  });
                                                  ...
                                                  let msg = format!("target {} {:?} dropped {cks} {reason}.",
                                                                    target.name(),
                                                                    target.kind());
            ...
            // run:
            {
                ...
                if ck.is_playdate() {
                    ...
                    let force = true;
                    // config.log().status("Running", "Starting!!!");
                    let fut = run_dev(query, pdx, no_install, no_read, force);
                    async_std::task::block_on(fut)?;
                    // config.log().status("Running", "ALL DONE");
                } else {
