...
pub fn build_all(config: &'_ Config,
    ...
    for ((package_id, _), mut products) in targets {
        log::debug!("Looking for assets artifacts for ({}) {}::{} for {}:",
                    &products[0].src_ct,
                    package_id,
                    products[0].name,
                    match &products[0].ck {
        ...
        let (root, assets) = assets.iter()
                                   ...
                                   .ok_or_else(|| {
                                       anyhow!("Root not found for ({}) {}::{} for {}",
                                               &products[0].src_ct,
                                               package_id,
                                               products[0].name,
                                               match products[0].ck {
                                                   CompileKind::Host => "host".to_owned(),
                                                   CompileKind::Target(kind) => kind.short_name().to_owned(),
                                               })
                                   })?;
...
fn package_single_target<'art>(config: &Config,
    ...
    config.log().status("Packaging",
                        format!("(single-target) {presentable_name} (source: {}, target: {})",
...
fn package_multi_target<'art>(config: &Config,
    ...
    config.log().status("Packaging",
                        format!("(multi-target) {} (sources: {src_cts}, targets: {dst_cts})",
    ...
    if let Some(assets) = assets {
        ...
        for artifact in assets {
            log::debug!("Packaging assets {:?} {}",
                        artifact.kind,
                        artifact.package_id.name());

            debug_assert_eq!(layout.as_ref().assets_layout(config).root(),
    ...
    // manifest:
    let cargo_target =
        (matches!(products[0].src_ct, CrateType::Bin) || products[0].example).then_some(products[0].name
                                                                                                   .as_str())
                                                                             .map(Cow::from);
    build_manifest(config,
                   &layout,
...
fn execute_pdc<'l, Layout: playdate::layout::Layout>(config: &Config,
    ...
    let (optimized, debuginfo) = match config.compile_options.build_config.requested_profile.as_ref() {
        ...
        _ => {
            ...
            let profiles = Profiles::new(&config.workspace,
