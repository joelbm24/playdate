    ...
    pub fn new(units: &'t UnitGraph, meta: &'t CargoMetadataPd) -> Self {
        ...
        for root in roots.iter_mut() {
            ...
            if deps_allowed {
                ...
                log::debug!("Total reduced deps for {}::{} ({:?}) : {}",
                            root.package_id().name(),
                            root.node.unit.target.name,
                            root.node.unit.target.kind,
                            deps.len());
            } else {
                log::debug!("No deps for {}::{} 🤷🏻‍♂️",
                            root.package_id().name(),
                            root.node.unit.target.name);
            }
            ...
            if root.node
                   ...
                   .is_some()
            {
                log::debug!("add root too because it has assets for {}",
                            root.node.unit.target.name);
            ...
            log::debug!("Total finally deps for {}::{} ({:?}) : {}",
                        root.package_id().name(),
                        root.node.unit.target.name,
                        root.node.unit.target.kind,
                        deps.len());
...
impl DependenciesAllowed for RootNode<'_> {
    fn deps_allowed(&self) -> bool {
        self.node.deps_allowed() || self.as_source().assets_options().dependencies()
    }
}
