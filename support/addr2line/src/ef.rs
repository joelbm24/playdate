    ...
    fn tags(&self) -> impl IntoIterator<Item = (&str, &str)> {
        if !self.is_empty() {
            [self.N().then_some(("N", Self::DOC_N)),
             ...
             self.GE().then_some(("GE", Self::DOC_GE))].into_iter()
                                                       .flatten()
        } else {
    ...
    fn tags(&self) -> impl IntoIterator<Item = (&str, &str)> {
        if !self.is_empty() {
            [self.TM().then_some(("TM", Self::DOC_TM)),
             ...
             self.UF().then_some(("UF", Self::DOC_UF))].into_iter()
                                                       .flatten()
        } else {
            <[_; 6]>::default().into_iter().flatten()
        }
    ...
    fn tags(&self) -> impl IntoIterator<Item = (&str, &str)> {
        if !self.is_empty() {
            [self.IS_NOT_ICI().then_some(("IS_NOT_ICI", Self::DOC_IS_NOT_ICI)),
             ...
             self.T().then_some(("T", Self::DOC_T))].into_iter()
                                                    .flatten()
        } else {
    ...
    fn tags(&self) -> impl IntoIterator<Item = (&str, &str)> {
        if !self.is_empty() {
            [self.VECTTBL().then_some(("VECTTBL", Self::DOC_VECTTBL)),
             ...
             self.DEBUGEVT().then_some(("DEBUGEVT", Self::DOC_DEBUGEVT))].into_iter()
                                                                         .flatten()
        } else {
...
impl RegTags for UFSR {
    ...
    fn tags(&self) -> impl IntoIterator<Item = (&str, &str)> {
        ...
        [self.UNDEFINSTR().then_some(("UNDEFINSTR", Self::DOC_UNDEFINSTR)),
         ...
         self.DIVBYZERO().then_some(("DIVBYZERO", Self::DOC_DIVBYZERO))].into_iter()
                                                                        .flatten()
    }
}
    ...
    fn tags(&self) -> impl IntoIterator<Item = (&str, &str)> {
        if !self.is_empty() {
            [self.IBUSERR().then_some(("IBUSERR", Self::DOC_IBUSERR)),
             ...
             self.LSPERR().then_some(("LSPERR", Self::DOC_LSPERR))].into_iter()
                                                                   .flatten()
        } else {
            <[_; 6]>::default().into_iter().flatten()
        }
...
impl RegTags for MMFSR {
    ...
    fn tags(&self) -> impl IntoIterator<Item = (&str, &str)> {
        ...
        [self.IACCVIOL().then_some(("IACCVIOL", Self::DOC_IACCVIOL)),
         ...
         self.MLSPERR().then_some(("MLSPERR", Self::DOC_MLSPERR))].into_iter()
                                                                  .flatten()
    }
}
