...
impl EnvResolver {
    ...
    pub fn cache(&self) -> Option<std::cell::Ref<BTreeMap<String, String>>> {
...
mod tests {
    ...
    fn resolve_includes_one_exact() {
        for file in ["Cargo.toml", "src/lib.rs"] {
            ...
            assert_eq!(Path::new(file).canonicalize().unwrap(),
    ...
    fn resolve_includes_one_glob() {
        for (file, expected) in [("Cargo.tom*", "Cargo.toml"), ("**/lib.rs", "src/lib.rs")] {
            ...
            assert_eq!(Path::new(expected).canonicalize().unwrap(),
                       matched.source().canonicalize().unwrap());
        }
    }
    ...
    fn resolve_includes_many_glob() {
        for (file, expected) in [("Cargo.*", &["Cargo.toml"][..]),
                                 ("**/*.rs", &["src/lib.rs", "src/assets/mod.rs"][..])]
        {
            ...
            for expected in expected {
                ...
                assert_eq!(expected.canonicalize().unwrap(),
                           matched.source().canonicalize().unwrap());
            }
    ...
    fn resolver_expr() {
        ...
        let exprs = [("${FOO}/file.txt", "foo/file.txt"),
